...
| `fm-bitbucket-api.sh`    | Call the fixed Bitbucket Cloud v2 API, reading or creating a pull request, with the default or repository-mapped bearer token supplied by environment, local `.env`, or Automic Vault without exposing it in arguments or files, and render the per-home Vault launcher for mapped repositories |
...
| `fm-pr-merge.sh`         | Record PR metadata and guard a canonical GitHub, GitLab, or Bitbucket Cloud merge; Bitbucket refuses submission because its API cannot atomically bind the verified head |
| `fm-pr-state.sh`         | Read-only: print one line per GitHub or Bitbucket Cloud pull-request blocker it can see, reporting observed checks rather than verdicting merge-readiness |
