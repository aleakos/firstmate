...
test_recovery_replays_a_bitbucket_close_as_a_body_link() {
  local case_dir id out
  id=atomic-heal-bitbucket-b9
  case_dir=$(make_home heal-bitbucket-close)
  add_item "$case_dir" "$id"
  start_item "$case_dir" "$id"
  printf 'id=%s\ndata=%s\nspawn_gen=spawn-heal-bitbucket\narg=--pr\narg=https://bitbucket.org/workspace/repository/pull-requests/17\n' \
    "$id" "$(home_of "$case_dir")/data" \
    > "$(home_of "$case_dir")/state/$id.backlog-close"

  out=$(run_bootstrap "$case_dir")
  [ "$(row_state "$case_dir" "$id")" = "done" ] \
    || fail "session start left an interrupted Bitbucket cleanup's item at $(row_state "$case_dir" "$id"): $out"
  assert_grep 'PR https://bitbucket.org/workspace/repository/pull-requests/17' "$(backlog_of "$case_dir")" \
    "the replayed close dropped the Bitbucket pull request the cleanup had recorded"
  assert_absent "$(home_of "$case_dir")/state/$id.backlog-close" \
    "a replayed Bitbucket close left its record behind"
  pass "session start replays a Bitbucket Cloud close with the pull request kept on the task"
}
