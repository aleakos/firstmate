...
config/bitbucket-repo-tokens config/bitbucket-av.sh  optional Bitbucket Cloud repository-to-Secret-Name map and the per-home blessable launcher rendered from it; LOCAL, gitignored, and not inherited; see docs/configuration.md "Per-repository access tokens"
config/commit-identity  optional per-project bot author and committer for ship and scout worker commits; LOCAL, gitignored, and not inherited; see docs/configuration.md "Worker commit identity"
